...
<div class="p-6 md:p-10">
    ...
    <div class="flex items-center mb-8">
        ...
        <h1 class="text-3xl font-extrabold text-gray-800" style="color: var(--secondary-color);">
            <i class="fas fa-user-edit ml-3 text-xl" style="color: var(--primary-color);"></i>
    ...
    <div class="card bg-white shadow-xl rounded-xl p-6 md:p-8">
        {{--
            * التغيير 1: تحديث الـ action ليرسل إلى دالة update.
            * التغيير 2: إضافة @method('PUT')
        --}}
        <form action="{{ route('admin.roles.update', $role->id) }}" method="POST">
            ...
            <div class="mb-6">
                <label for="name" class="block text-lg font-medium text-gray-700 mb-2">اسم الدور (باللغة
                    الإنجليزية)</label>
                {{-- التغيير 3: وضع القيمة الحالية للدور في حقل الإدخال --}}
            ...
            <div class="mb-8">
                ...
                {{-- يُفترض أن المتحكم يمرر $permissions (جميع الصلاحيات) و $rolePermissions (صلاحيات الدور الحالي) --}}
                @if(isset($permissions) && $permissions->isNotEmpty())

                    {{-- تجميع الصلاحيات حسب المورد --}}
                    @php
                        // استخراج IDs الصلاحيات الممنوحة للدور الحالي
                        $rolePermissionsIds = $role->permissions->pluck('id')->toArray();

                        // تجميع جميع الصلاحيات حسب المورد
                                    ...
                                    @foreach ($perms as $permission)
                                        <div class="flex items-center">
                                            <input type="checkbox" id="perm_{{ $permission->id }}" name="permissions[]"
                                                value="{{ $permission->id }}"
                                                {{-- التغيير 4: التحقق مما إذا كانت الصلاحية ممنوحة للدور الحالي --}}
    ...
    <!-- JavaScript لتشغيل وظائف الاختيار/الإلغاء (لم يتغير) -->
    <script>
        document.addEventListener('DOMContentLoaded', function () {
            ...
            // نستخدم setTimeout هنا لنمنح DOM الوقت الكافي للرسم قبل تحديث المربعات بناءً على حالة checked
            setTimeout(updateGroupAndMasterCheckboxes, 0); 
        });
    </script>
</div>
