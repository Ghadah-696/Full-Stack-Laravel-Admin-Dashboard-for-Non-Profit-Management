    ...
    <div class="p-6 md:p-10">
        ...
        <div class="flex justify-between items-center mb-8">
            <h1 class="text-3xl font-extrabold text-gray-800" style="color: var(--secondary-color);">
                <i class="fas fa-edit ml-2" style="color: var(--primary-color);"></i> تعديل تصنيف حالي
            </h1>
        ...
        <form action="{{ route('admin.categories.update', $categories->id) }}" method="POST" enctype="multipart/form-data"
            ...
            <div class="mb-6">
                ...
                <input
                    ...
                    id="name_ar" type="text" name="name_ar" value="{{ $categories->name_ar }}"
            ...
            <div class="mb-6">
                ...
                <input
                    class="form-input w-full px-4 py-3 border border-gray-300 rounded-lg focus:outline-none focus:ring-2 focus:ring-blue-500 transition duration-150 ltr:text-left rtl:text-right"
                    id="name_en" type="text" name="name_en" value="{{ $categories->name_en }}"
            ...
            <div>
                ...
                <textarea name="description_ar" id="description_ar" rows="3"
                    class="mt-1 block w-full rounded-md border-gray-300 shadow-sm focus:border-indigo-500 focus:ring-indigo-500 p-2 border">{{ old('description_ar', $categories->description_ar) }}</textarea>
            ...
            <div>
                ...
                <textarea name="description_en" id="description_en" rows="3"
                    class="mt-1 block w-full rounded-md border-gray-300 shadow-sm focus:border-indigo-500 focus:ring-indigo-500 p-2 border">{{ old('description_en', $categories->description_en) }}</textarea>
                @error('description_en')
                    <p class="text-red-500 text-xs mt-1">{{ $message }}</p>
                @enderror
            </div>

            <!-- حقل النوع (القائمة المنسدلة مع القيمة الحالية) -->
            <div>
                <label for="type" class="block text-sm font-medium text-gray-700">نوع التصنيف</label>
                @php
                    $currentType = old('type', $categories->type);
                @endphp
                <select name="type" id="type" required
                    ...
                    <option value="project" {{ $currentType == 'project' ? 'selected' : '' }}>للمشاريع (Project)</option>
                    <option value="news" {{ $currentType == 'news' ? 'selected' : '' }}>للأخبار (News)</option>
                </select>
                @error('type')
                    <p class="text-red-500 text-xs mt-1">{{ $message }}</p>
                @enderror
